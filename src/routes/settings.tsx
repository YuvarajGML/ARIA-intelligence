      ...
      <div className="space-y-6 max-w-3xl">
          ...
          <div className="space-y-2">
            {SECRETS.map((s) => {
              ...
              return (
                <div
                  key={s.name}
                  className="flex items-center gap-3 p-3 rounded border border-border bg-background/40"
                >
                  {ok ? (
                  ...
                  <div
                    className={`font-mono text-[10px] uppercase tracking-wider ${ok ? "text-success" : "text-warn"}`}
                  >
        ...
        <div className="rounded-xl border border-border bg-card/60 p-5">
          ...
          <p className="text-sm text-muted-foreground">
            ARIA runs in <span className="text-primary font-mono">mock-engine</span> mode by default
            — a complete multi-agent pipeline running in TanStack server functions. Set{" "}
            <code className="font-mono text-xs bg-background px-1 py-0.5 rounded">
              ARIA_BACKEND_URL
            </code>{" "}
            to forward runs to a Python LangGraph backend instead.
          </p>
        </div>
      </div>
    </AppShell>
